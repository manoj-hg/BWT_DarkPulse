...
class SimpleFaceRecognitionService:
    """Simplified face recognition service using OpenCV's built-in face detection"""
    ...
    def load_known_faces(self):
        """Load known faces from dataset folder using simple face detection"""
        try:
            ...
            while self.is_running:
                ...
                for (x, y, w, h) in faces:
                    ...
                    for name, face_data in self.known_faces.items():
                        ...
                        result = cv2.matchTemplate(face_roi, face_data['face_data'], cv2.TM_SQDIFF)
                        min_val, _, _, _ = cv2.minMaxLoc(result)
                        
                        if min_val < best_score and min_val < 50000:  # Threshold for matching
                            best_score = min_val
                            best_match = name
                    ...
                    if best_match:
                        ...
                        if (best_match not in recently_recognized or 
                            ...
                            self._draw_face_info(frame, x, y, w, h, best_match, True)
                        else:
                            # Draw rectangle but don't mark attendance again
                            self._draw_face_info(frame, x, y, w, h, best_match, False)
                    else:
                        # Draw rectangle for unknown face
                        self._draw_face_info(frame, x, y, w, h, "Unknown", False)
    ...
    def _draw_face_info(self, frame, x, y, w, h, name, marked_attendance):
